...
def test_Matrix_instance():
    '''
    Create instance of Matrix parent class, for use in unittests.
    '''
...
def test_MatrixSparse_instance():
    '''
    Create instance of MatrixSparse subclass, for use in unittests.
    '''
...
def test_MatrixRows_instance():
    '''
    Create instance of MatrixRows subclass, for use in unittests.
    '''
...
def test_MatrixCols_instance():
    '''
    Create instance of Matrix subclass, for use in unittests.
    '''
...
class TestMatrix(unittest.TestCase):
    ...
    def test_changematrixshape(self):
        x = test_Matrix_instance()
        self.assertEqual(x.change_matrix_shape(MatrixCols).get_elements(),\
            [[1, 4, 7], [2, 5, 8], [3, 6, 9]])
        self.assertEqual(x.change_matrix_shape(MatrixRows).get_elements(),\
            [[1, 2, 3], [4, 5, 6], [7, 8, 9]])
        self.assertEqual(x.change_matrix_shape(MatrixSparse).get_elements(),\
    ...
    def test_getvalue(self):
        ...
        self.assertEqual(x1.get_value(2,2), 9)
        self.assertEqual(x2.get_value(2,2), 3)
        self.assertEqual(x3.get_value(1,1), 5)
        self.assertEqual(x4.get_value(1,1), 5)

    def test_transpose(self):
        x1 = test_MatrixRows_instance()
        x2 = test_MatrixCols_instance()
        self.assertEqual(x1.transpose().get_elements(), [[1, 4, 7], [2, 5, 8],\
            [3, 6, 9]])
        self.assertEqual(x2.transpose().get_elements(), [[1, 2, 3], [4, 5, 6],\
            [7, 8, 9]])
    ...
    def test_addmatrix(self):
        ...
        self.assertEqual(x1.add_matrix(other1).get_elements(), [[2, 4, 6],\
            [8, 10, 12], [14, 16, 18]])
        self.assertEqual(x2.add_matrix(other2).get_elements(), [[1, 2, 3],\
            [7, 5, 6], [12, 8, 12]])
        self.assertEqual(x1.add_matrix(other3).get_elements(), [[1, 2, 3],\
            [7, 5, 6], [12, 8, 12]])
    ...
    def test_subtractmatrix(self):
        ...
        other2 = test_MatrixCols_instance()
        other3 = test_MatrixSparse_instance()
        self.assertEqual(x1.subtract_matrix(other1).get_elements(),\
            [[0, 0, 0], [0, 0, 0], [0, 0, 0]])
        self.assertEqual(x2.subtract_matrix(other2).get_elements(),\
            [[-1, -2, -3], [-1, -5, -6], [-2, -8, -6]])
        self.assertEqual(x1.subtract_matrix(other3).get_elements(),\
            [[1, 2, 3], [1, 5, 6], [2, 8, 6]])
    ...
    def test_scalar_multiply(self):
        ...
        x3 = test_MatrixRows_instance()
        x4 = test_MatrixCols_instance()
        self.assertEqual(x1.scalar_multiply(-1).get_elements(),\
            [-1, -2, -3, -4, -5, -6, -7, -8, -9])
        self.assertEqual(x2.scalar_multiply(2).get_elements(),\
            {(1, 0): 6, (2, 0): 10, (2, 2): 6})
        self.assertEqual(x3.scalar_multiply(0).get_elements(),\
            [[0, 0, 0], [0, 0, 0], [0, 0, 0]])
        self.assertEqual(x4.scalar_multiply(-2).get_elements(),\
    ...
    def test_scalar_divide(self):
        ...
        x3 = test_MatrixRows_instance()
        x4 = test_MatrixCols_instance()
        self.assertEqual(x1.scalar_divide(-1).get_elements(),\
            [-1, -2, -3, -4, -5, -6, -7, -8, -9])
        self.assertEqual(x2.scalar_divide(2).get_elements(),\
            {(1, 0): 1.5, (2, 0): 2.5, (2, 2): 1.5})
        self.assertEqual(x3.scalar_divide(1).get_elements(),\
            [[1, 2, 3], [4, 5, 6], [7, 8, 9]])
        self.assertEqual(x4.scalar_divide(-2).get_elements(),\
    ...
    def test_slicematrix(self):
        ...
        x3 = test_MatrixRows_instance()
        x4 = test_MatrixCols_instance()
        self.assertEqual(x1.slice_matrix(0, 2, 1, 2), [[2, 3, 4],\
            [4, 5, 6], [7, 8, 9]])
    ...
    def test_dotproduct(self):
        ...
        self.assertEqual(x1.dot_product(slice1, slice3, 0, 0, x1.num_cols, 0,\
            1, x3.num_rows), 36)
        self.assertEqual(x2.dot_product(slice2, slice3, 2, 0, x2.num_cols, 0,\
            1, x3.num_rows), 34)
    ...
    def test_multiplymatrix(self):
        ...
        x3 = test_MatrixRows_instance()
        x4 = test_MatrixCols_instance()
        self.assertEqual(x1.multiply_matrix(x3).get_elements(), [[30, 36, 42],\
            [66, 81, 96], [102, 126, 150]])
        self.assertEqual(x2.multiply_matrix(x4).get_elements(), [[0, 0, 0],\
            [3, 6, 9], [26, 34, 42]])
...
class TestMatrixSparse(unittest.TestCase):
    ...
    def test_changematrixshape(self):
        ...
        self.assertEqual(x.change_matrix_shape(MatrixCols).get_elements(),\
            [[0, 3, 5], [0, 0, 0], [0, 0, 3]])
        self.assertEqual(y.change_matrix_shape(MatrixRows).get_elements(),\
            [[0, 0, 0], [3, 0, 0], [5, 0, 3]])
